#!/usr/bin/env node
// Fetches market data + watchlist prices, writes public/data.json.
// Runs server-side (GitHub Actions or locally with `node scripts/fetch-data.mjs`)
// so it never hits browser CORS/rate-limit blocks from Yahoo Finance.
// Metrics and symbols are defined in config/*.json — edit those to add/remove/change
